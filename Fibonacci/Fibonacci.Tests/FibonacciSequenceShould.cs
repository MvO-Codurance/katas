...
public class FibonacciSequenceShould
{
    ...
    [InlineAutoNSubstituteData(10, 55)]
    [InlineAutoNSubstituteData(15, 610)]
    public void Determine_The_Correct_Number_At_The_Given_Step_Recursive(int step, int expectedNumber)
    {
        new FibonacciSequence().NumberAtStepRecursive(step).Should().Be(expectedNumber);
    }
}